...
export default class calendarDataProvider {
  constructor() {
    ...
    this.currentMonthDataMatrix =
    chunk(this.getCalendarData(this.currentYear, this.currentMonth), 7);
  ...
  getCurrentWeek() {
    const todayWeekIndex = this.currentMonthDataMatrix.findIndex((week, index) => {
      let toReturn = false;
      week.forEach((elem) => {
        if (elem.date === this.currentDate) {
          if (!(index === 0 && this.currentDate > 8)) {
            toReturn = true;
          }
        }
      });
  ...
  getCalendarData(year, month) {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      calendarDataArray.push(
        {
          date: i, day: tempDate.getDay(), month: tempDate.getMonth(), year: tempDate.getFullYear(),
        });
    }
